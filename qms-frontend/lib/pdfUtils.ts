...
export const generateEnhancedQuotationPDF = async (quotationData: EnhancedQuotationData): Promise<void> => {
  try {
    ...
    // Customer Information Box (Top Left) - matching the image layout
    const headerHeight = 20;
    const leftBoxWidth = pageWidth / 2 - margin - 5;

    pdf.setDrawColor(0, 0, 0);
    pdf.rect(margin, yPosition, leftBoxWidth, headerHeight);

    pdf.setFontSize(12);
    pdf.setFont('helvetica', 'bold');
    pdf.text(quotationData.companyName || 'Customer Name', margin + 2, yPosition + 8);
    pdf.setFontSize(10);
    pdf.setFont('helvetica', 'normal');
    pdf.text(quotationData.companyAddress || 'Customer Address', margin + 2, yPosition + 15);

    // Date and Ref boxes on the right - matching the image
    const rightBoxX = pageWidth / 2 + 5;
    const rightBoxWidth = pageWidth - rightBoxX - margin;

    // Date row
    drawCell(rightBoxX, yPosition, 25, 10, 'Date', 'left', 10, true);
    drawCell(rightBoxX + 25, yPosition, rightBoxWidth - 25, 10, quotationData.date, 'center');

    // Ref.No row  
    drawCell(rightBoxX, yPosition + 10, 25, 10, 'Ref.No', 'left', 10, true);
    drawCell(rightBoxX + 25, yPosition + 10, rightBoxWidth - 25, 10, quotationData.refNo, 'center');

    yPosition += headerHeight + 10;

    // Quotation For section (centered) - matching the image
    if (quotationData.quotationFor) {
      const quotationForText = `Quotation for ( ${quotationData.quotationFor} )`;
      const quotationBoxWidth = pageWidth - 2 * margin;

      pdf.setDrawColor(0, 0, 0);
      pdf.rect(margin, yPosition, quotationBoxWidth, 10);

      pdf.setFontSize(11);
      pdf.setFont('helvetica', 'bold');
      pdf.text(quotationForText, pageWidth / 2, yPosition + 7, { align: 'center' });
    ...
    // Table headers - matching the image format (total: 144mm)
    const colWidths = [15, 55, 15, 15, 25, 25, 20]; // Sr.No, Description, UOM, Qty, Unit Price, Total Price, GST Rate
    ...
    // Terms & Conditions - matching the image format
    pdf.setFontSize(12);
    pdf.setFont('helvetica', 'bold');
    pdf.text('Terms & Conditions:', margin, yPosition);
    yPosition += 10;
    ...
    if (quotationData.termsConditions?.fullText) {
      // Use dynamic terms from the form
      const termsLines = quotationData.termsConditions.fullText.split('\n');
      termsLines.forEach(line => {
        if (line.trim()) {
          pdf.text(line.trim(), margin + 20, yPosition);
          yPosition += 6;
        }
      });
    } else {
      // Fallback to structured terms matching the image
    ...
    pdf.setFontSize(8);
    pdf.setFont('helvetica', 'normal');
    pdf.text(`For ${quotationData.companyName || 'Anoosh International'}`, margin, yPosition + 8);

    // Save the PDF
    const companyNameForFile = (quotationData.companyName || 'Anoosh_International').replace(/[^a-zA-Z0-9]/g, '_');
    const filename = `${companyNameForFile}_Enhanced_Quotation_${quotationData.refNo}_${new Date().toISOString().split('T')[0]}.pdf`;
    pdf.save(filename);
